...
import { capitalize, pluckString, slugify } from "@/utils/string";
...
const BASE_ENDPOINT = "/api/cms/news";
const PREFIX_ID = "cms_news";
type Interface__Data = Interface__CMSNews;
...
const Create = (props: any) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object().shape({
      ...
      slugId: yup.string().required(l.msg_required_form),
      slugEn: yup.string().required(l.msg_required_form),
  ...
  useEffect(() => {
    formik.setFieldValue("slugId", slugify(formik.values.titleId));
  }, [formik.values.titleId]);

  useEffect(() => {
    formik.setFieldValue("slugEn", slugify(formik.values.titleEn));
              ...
              <FieldsetRoot disabled={loading}>
                <SimpleGrid columns={[1, null, 2]} gap={4}>
                  ...
                  <CContainer gap={4}>
                    ...
                    <Field
                      label={"Slug"}
                      invalid={!!(formik.errors.slugId || formik.errors.slugEn)}
                      errorText={
                        (formik.errors.slugId || formik.errors.slugEn) as string
                      }
                    >
                      <InputGroup
                        startElement="id"
                      ...
                      >
                        <StringInput
                          inputValue={formik.values.slugId}
                          onChange={(inputValue) => {
                            formik.setFieldValue("slugId", inputValue);
                          }}
                        />
                      </InputGroup>
                      <InputGroup
                      ...
                      >
                        <StringInput
                          inputValue={formik.values.slugEn}
                          onChange={(inputValue) => {
                            formik.setFieldValue("slugEn", inputValue);
                          }}
                        />
                      </InputGroup>
                    </Field>
...
const Update = (props: any) => {
  ...
  useEffect(() => {
    formik.setValues({
      category: [
        {
          id: resolvedData.newsCategory.id,
          label: resolvedData.newsCategory.name[lang],
        },
      ],
      files: [],
      titleId: resolvedData.title.id,
      titleEn: resolvedData.title.en,
      slugId: resolvedData.slug.id,
      slugEn: resolvedData.slug.en,
      descriptionId: resolvedData.description.id,
      descriptionEn: resolvedData.description.en,
      deleteDocumentIds: [],
      contentId: resolvedData.newsContent.id,
      contentEn: resolvedData.newsContent.en,
    });
  }, [open, resolvedData]);
...
const Data = (props: any) => {
  ...
  const dataProps: Interface__DataProps = {
    ...
    rows: data?.map((item, idx) => ({
      ...
      columns: [
        {
          td: <ClampText>{`${item.newsCategory?.name?.[lang]}`}</ClampText>,
          value: item.newsCategory?.name?.[lang],
        },
        {
          td: (
            <ImgViewer
