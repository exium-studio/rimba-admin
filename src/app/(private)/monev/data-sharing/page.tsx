...
const BASE_ENDPOINT = "/api/monev/share-report";
const PREFIX_ID = "monev_data_sharing";
type Interface__Data = Interface__MonevDataSharing;
...
const FilesList = (props: any) => {
  ...
  const { open, onOpen, onClose } = useDisclosure();
  useBackOnClose(
    disclosureId(`data-sharing-list-${data.id}`),
    open,
    onOpen,
    onClose
  );
    ...
    <>
      ...
      <DisclosureRoot open={open} lazyLoad size={"xs"}>
        <DisclosureContent>
          ...
          <DisclosureBody>
            <CContainer gap={4}>
              {data?.report?.map((doc: Interface__StorageFile) => {
